...
CREATE TYPE "public"."platform" AS ENUM('discord', 'youtube', 'twitch', 'facebook', 'twitter', 'instagram', 'website', 'email', 'other');--> statement-breakpoint
...
ALTER TABLE "carousel" ADD CONSTRAINT "carousel_orgId_club_id_fk" FOREIGN KEY ("orgId") REFERENCES "public"."club"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE VIEW "public"."used_tags" AS (select UNNEST("club"."tags") as tags, COUNT("club"."tags") as count from club group by UNNEST("club"."tags") order by count desc);